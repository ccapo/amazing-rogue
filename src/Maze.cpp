...
void Maze::createMaze() {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      int offset = x + y*width;
      if (maze_tiles[offset] == FLOOR) floor_tiles.push_back(offset);
    }
  }
